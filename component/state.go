...
func (sm *StateManager) SaveScheduleState(scheduleState ScheduleState) error {
	...
	if err := dryrun.Call("InputBoolean.TurnOn", sm.config.IsScheduledEntity, func() error {
		return sm.service.InputBoolean.TurnOn(sm.config.IsScheduledEntity)
	}); err != nil {
	...
	if err := dryrun.CallWithData("HomeAssistant.TurnOn", sm.config.ModeEntity, scheduleState.Mode, func() error {
		return sm.service.HomeAssistant.TurnOn(
	...
	// Save start time
	if err := dryrun.CallWithData("InputDatetime.Set", sm.config.StartTimeEntity, scheduleState.StartTime, func() error {
		return sm.service.InputDatetime.Set(
	...
	// Save costs
	if err := dryrun.CallWithData("InputNumber.Set", sm.config.EstimatedCostEntity, scheduleState.EstimatedCost, func() error {
		return sm.service.InputNumber.Set(sm.config.EstimatedCostEntity, float32(scheduleState.EstimatedCost))
	}); err != nil {
		return fmt.Errorf("failed to save estimated cost: %w", err)
	}

	if err := dryrun.CallWithData("InputNumber.Set", sm.config.CurrentCostEntity, scheduleState.CurrentCost, func() error {
		return sm.service.InputNumber.Set(sm.config.CurrentCostEntity, float32(scheduleState.CurrentCost))
	}); err != nil {
		return fmt.Errorf("failed to save current cost: %w", err)
	}

	if err := dryrun.CallWithData("InputNumber.Set", sm.config.SavingsPercentEntity, scheduleState.SavingsPercent, func() error {
		return sm.service.InputNumber.Set(sm.config.SavingsPercentEntity, float32(scheduleState.SavingsPercent))
	}); err != nil {
	...
	if sm.config.DelayEntity != "" {
		if err := dryrun.CallWithData("InputNumber.Set", sm.config.DelayEntity, scheduleState.DelayMinutes, func() error {
			return sm.service.InputNumber.Set(sm.config.DelayEntity, float32(scheduleState.DelayMinutes))
		}); err != nil {
	...
	if sm.config.OptimalTimeEntity != "" {
		if err := dryrun.CallWithData("InputText.Set", sm.config.OptimalTimeEntity, scheduleState.OptimalTimeStr, func() error {
			return sm.service.InputText.Set(sm.config.OptimalTimeEntity, scheduleState.OptimalTimeStr)
		}); err != nil {
	...
	if sm.config.SavingsEntity != "" {
		if err := dryrun.CallWithData("InputText.Set", sm.config.SavingsEntity, scheduleState.SavingsStr, func() error {
			return sm.service.InputText.Set(sm.config.SavingsEntity, scheduleState.SavingsStr)
		}); err != nil {
...
func (sm *StateManager) ClearScheduleState() error {
	...
	if err := dryrun.Call("InputBoolean.TurnOff", sm.config.IsScheduledEntity, func() error {
		return sm.service.InputBoolean.TurnOff(sm.config.IsScheduledEntity)
	}); err != nil {
		return fmt.Errorf("failed to clear scheduled flag: %w", err)
	}

	// Reset mode to "none" value
	if err := dryrun.CallWithData("HomeAssistant.TurnOn", sm.config.ModeEntity, sm.config.ModeNoneValue, func() error {
		return sm.service.HomeAssistant.TurnOn(
			sm.config.ModeEntity,
			map[string]interface{}{
	...
	if err := dryrun.CallWithData("InputDatetime.Set", sm.config.StartTimeEntity, time.Time{}, func() error {
		return sm.service.InputDatetime.Set(sm.config.StartTimeEntity, time.Time{})
	}); err != nil {
		log.Printf("WARNING: Failed to reset start time: %v", err)
	}

	// Reset costs
	if err := dryrun.CallWithData("InputNumber.Set", sm.config.EstimatedCostEntity, 0, func() error {
		return sm.service.InputNumber.Set(sm.config.EstimatedCostEntity, 0)
	}); err != nil {
		log.Printf("WARNING: Failed to reset estimated cost: %v", err)
	}

	if err := dryrun.CallWithData("InputNumber.Set", sm.config.CurrentCostEntity, 0, func() error {
		return sm.service.InputNumber.Set(sm.config.CurrentCostEntity, 0)
	}); err != nil {
		log.Printf("WARNING: Failed to reset current cost: %v", err)
	}

	if err := dryrun.CallWithData("InputNumber.Set", sm.config.SavingsPercentEntity, 0, func() error {
		return sm.service.InputNumber.Set(sm.config.SavingsPercentEntity, 0)
	}); err != nil {
	...
	if sm.config.DelayEntity != "" {
		if err := dryrun.CallWithData("InputNumber.Set", sm.config.DelayEntity, 0, func() error {
			return sm.service.InputNumber.Set(sm.config.DelayEntity, 0)
		}); err != nil {
	...
	if sm.config.OptimalTimeEntity != "" {
		if err := dryrun.CallWithData("InputText.Set", sm.config.OptimalTimeEntity, "N/A", func() error {
			return sm.service.InputText.Set(sm.config.OptimalTimeEntity, "N/A")
		}); err != nil {
	...
	if sm.config.SavingsEntity != "" {
		if err := dryrun.CallWithData("InputText.Set", sm.config.SavingsEntity, "€0.00", func() error {
			return sm.service.InputText.Set(sm.config.SavingsEntity, "€0.00")
		}); err != nil {
